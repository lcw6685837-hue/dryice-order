...
function triggerSmartOrderAlert() {
  ...
  if (isInputActive) {
    // 영업팀 사용자가 셀을 편집 중이라면 팝업 대기 설정 (입력 방해 차단)
    pendingOrderAlert = true;
  } else {
    // 입력 중이 아니라면 (현장 모니터링 등) 즉시 대형 팝업 출력
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // 🛡️ 3. Firebase 실시간 동기화 커넥션 코어 (주문 감지 및 팝업 정밀 필터링 적용)
