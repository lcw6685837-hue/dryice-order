...
function toggleFcSection() {
  ...
  const text = document.getElementById("text-toggle-fc");

  if (!fcBox) return;

  fcBox.classList.toggle("collapsed");
  const isCollapsed = fcBox.classList.contains("collapsed");
...
// 🚨 스마트 포커스 감지형 알림 상태 및 변동 거래처 / 타임스탬프 추적 변수
let isInitialLoad = true;
let previousOrders = {};
let pendingOrderAlert = false;
let pendingChangedClients = new Set();
let lastOrderChangeDateObj = null;
let lastOrderChangeFormattedStr = "";

// 🔔 실시간 저장 및 연결 상태 알림 토스트 생성기
function showToast(message, type = "error") {
  ...
  if (!container) {
    ...
    container.className =
      "fixed top-4 right-4 z-50 flex flex-col gap-2 no-print";
  ...
  toast.className = `px-4 py-3 rounded-xl border-2 shadow-2xl backdrop-blur-md text-xs font-bold flex items-center gap-2 animate-bounce ${bgColor}`;
  toast.innerHTML = `<i class="fas ${type === "error" ? "fa-exclamation-triangle" : "fa-check-circle"} text-base"></i><span>${message}</span>`;
...
function showOrderAlert() {
  ...
  if (tagsContainer) {
    if (pendingChangedClients.size > 0) {
      tagsContainer.innerHTML = Array.from(pendingChangedClients)
        .map(
          (clientName) =>
            `<div class="px-5 py-2 bg-slate-950/90 border-2 border-amber-400 text-amber-300 font-extrabold text-lg md:text-xl rounded-xl shadow-[0_0_15px_rgba(245,158,11,0.4)] tracking-wide animate-pulse">
                    ${clientName}
                </div>`,
        )
        .join("");
...
function triggerSmartOrderAlert() {
...
const CLIENTS = [
  ...
  { name: "화일상사(2)", dest: "용인" },
  { name: "영재상사", dest: "서울" },
  { name: "한국콜드체인(1)", dest: "이천" },
  { name: "한국콜드체인(2)", dest: "이천" },
  { name: "경기남부", dest: "평택" },
  { name: "용인드라이", dest: "서산" },
...
const FC_LEFT = [
  ...
  { id: "fc_kcc_1", label: "한국콜드체인(1)", type: "normal" },
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  connectedRef.on("value", (snap) => {
    if (snap.val() === false) {
      showToast(
        "네트워크 연결이 끊어졌습니다. 데이터가 저장되지 않을 수 있습니다.",
        "error",
      );
    }
  });
  ...
  function setOrderPhase(phase, saveToDb = true) {
    ...
    } else if (phase === "4" && btn4) {
      btn4.className =
        "phase-btn px-2 py-0.5 rounded-full text-[11px] font-extrabold bg-yellow-500/20 text-yellow-400 border border-yellow-500/40 hover:bg-yellow-500/30 transition flex items-center gap-1";
      btn4.querySelector("span").className =
    ...
    if (saveToDb && currentRef) {
      currentRef
        ...
        .set(phase)
        .catch((err) => {
          showToast("차수 변경 저장 실패: " + err.message, "error");
        });
    }
  }

  if (document.getElementById("btn-phase-1"))
    document
      .getElementById("btn-phase-1")
      .addEventListener("click", () => setOrderPhase("1"));
  if (document.getElementById("btn-phase-2"))
    document
      .getElementById("btn-phase-2")
      .addEventListener("click", () => setOrderPhase("2"));
  if (document.getElementById("btn-phase-3"))
    document
      .getElementById("btn-phase-3")
      .addEventListener("click", () => setOrderPhase("3"));
  if (document.getElementById("btn-phase-4"))
    document
      .getElementById("btn-phase-4")
      .addEventListener("click", () => setOrderPhase("4"));
  ...
  document.addEventListener("focusout", () => {
    if (pendingOrderAlert) {
      setTimeout(() => {
        ...
        const isStillInputting =
          nextActive &&
          (nextActive.tagName === "INPUT" || nextActive.tagName === "TEXTAREA");

        if (!isStillInputting && pendingOrderAlert) {
          pendingOrderAlert = false;
          showOrderAlert();
        }
      }, 200);
      ...
      if (clientCell) {
        if (hasUnproduced) {
          clientCell.className =
            "client-cell !bg-lime-500/20 !text-lime-400 font-black border-2 border-lime-400/60 shadow-[0_0_15px_rgba(132,204,22,0.4)] transition-all duration-300";
        } else {
  ...
  function getActiveInputs() {
    return Array.from(
      document.querySelectorAll(
        "input.qty-input, input.fc-input, .fc-sub-input, textarea.sync-item, .remark-cell textarea",
      ),
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (
      !target.matches(
        "input.qty-input, input.fc-input, .fc-sub-input, textarea",
      )
    )
      return;
  ...
  function loadLogData(dateStr) {
    ...
    currentRef.on(
      ...
      (snapshot) => {
        ...
        syncItems.forEach((item) => {
          if (document.activeElement !== item) {
            ...
            item.value =
              dbValue === null || dbValue === undefined || dbValue === "null"
                ? ""
                : dbValue;
          }
          ...
          if (item.id.endsWith("_order")) {
            ...
            if (
              !isInitialLoad &&
              previousOrders[item.id] !== undefined &&
              previousOrders[item.id] !== currentVal &&
              currentVal !== ""
            ) {
              ...
              if (match) {
                ...
                if (CLIENTS[ci]) {
                  pendingChangedClients.add(CLIENTS[ci].name);
                }
              }
            }
            previousOrders[item.id] = currentVal;
        ...
        if (!isInitialLoad && hasOrderChanged) {
          ...
          const mm = String(lastOrderChangeDateObj.getMinutes()).padStart(
            2,
            "0",
          );
      ...
      (error) => {
        showToast("데이터 불러오기 실패: " + error.message, "error");
      },
    );
  }
