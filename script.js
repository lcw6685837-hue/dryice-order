...
document.addEventListener('DOMContentLoaded', () => {
    ...
    CLIENTS.forEach((client, ci) => {
        ROW_TYPES.forEach((rt, ri) => {
            ...
            tr.className = rt.cls + (ci % 2 === 0 ? ' client-even' : ' client-odd') + (ri === 0 ? ' client-start' : '') + ` client-row-${ci}`;
    ...
    document.addEventListener('focusout', () => {
        if (pendingOrderAlert) {
            setTimeout(() => {
                ...
                const isStillInputting = nextActive && (nextActive.tagName === 'INPUT' || nextActive.tagName === 'TEXTAREA');
